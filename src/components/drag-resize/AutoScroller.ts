  ...
  start = () => {
    ...
    const scrollFunc = () => {
      if (this.scroller && this.clientX !== null) {
        if (this.clientX + this.space > this.scroller?.getBoundingClientRect().right) this.handleScroll('right')
        else if (this.clientX - this.space < this.scroller?.getBoundingClientRect().left) this.handleScroll('left')
      }
